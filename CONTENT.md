# DataDoodles Website Content

This file collects the website’s editable words in page order.

Edit the wording under each heading, keeping the section headings and field labels intact so the changes can be mapped back to the Astro files. After editing, ask Codex to apply `CONTENT.md` to the website.

---

## 1. SEO and Sharing Metadata

### Page title

DataDoodles — Get found in Google and AI search

### Meta description

We rebuild Malaysian SME websites so customers find you in Google and ChatGPT, trust you in 10 seconds, and reach you in one tap.

### Structured-data organization description

DataDoodles rebuilds Malaysian SME websites for visibility in Google and AI search.

### Structured-data service name

The 30-Day Findable Site

### Structured-data service area

Malaysia

### Structured-data local area

Klang Valley, Malaysia

### Structured-data audience

Established Malaysian SMEs

### Structured-data offer

- Price: 14800
- Currency: MYR

---

## 2. Navigation Bar

### Logo accessibility label

DataDoodles home

### Navigation accessibility label

Sections

### Desktop and mobile navigation links

1. The problem
2. The shift
3. What we build
4. 30 days
5. Proof
6. Pricing
7. FAQ

### Header call to action

Get a consultation

### Mobile menu controls

- Open menu
- Close menu
- Sections menu

### Mobile menu call to action

Get a consultation

---

## 3. Hero

### Kicker

Get your website discovered in 30 days

### Main headline

Your competitors are not always better. They are just easier to choose online.

### Supporting paragraph

We optimize your website so customers find you in Google and ChatGPT, trust you in 10 seconds, and reach you in one tap. All within 30 days.

### Benefit points

- Get discovered by AI in 30 days
- Optimized website conversion
- AEO and GEO optimization

### Hero form label

Free Website Audit

### Hero form headline

Get a 30 minutes consultation you can be discovered by AI in 30-days

### Hero GEO chat simulator

- Title: AI Overview
- Question: Which clinic in Puchong is best for paediatric dentistry?
- Response group 1: When looking for the best paediatric dental care in Puchong,
- Response group 2: there is a mix of specialist clinics and family-oriented practices that go out of their way to make children feel comfortable.
- Response group 3: Here are the top recommendations:
- Recommendation 1: Your Business
- Recommendation 1 label: RECOMMENDED
- Recommendation 2: Competitor 1
- Recommendation 3: Competitor 2

### Hero marquee — large row

- GEO
- AEO
- SEO

### Hero marquee — small row

- Easy to find
- Easy to trust
- Easy to contact

### Screen-reader summary for the marquee

DataDoodles builds for GEO, AEO and SEO. Easy to find, easy to trust, easy to contact.

---

## 4. The Problem

### Navigation anchor

The problem

### Section label

The cost of being hard to choose

### Section number

01

### Section headline

Four ways you’re losing customers you never knew you had.

### Problem 01

#### Heading

They didn’t know what you actually do.

#### Copy

Someone landed on your homepage. Ten seconds later they left, still unsure whether you were the right fit for their exact problem. They didn’t ask. They just went back to Google.

### Problem 02

#### Heading

They couldn’t tell if you were any good.

#### Copy

No case studies. No real faces. No numbers. Just words claiming you’re professional and trusted. So they picked the competitor who showed them instead of telling them.

### Problem 03

#### Heading

They never found you in the first place.

#### Copy

They asked ChatGPT for the best options in their area. They searched Google Maps. Your business exists — but the search engines and AI models couldn’t understand you well enough to recommend you. Someone else got named.

### Problem 04

#### Heading

They wanted to contact you and gave up.

#### Copy

The form asked for nine fields. The WhatsApp button was buried at the bottom. Nobody replied for two days. They were ready to buy, and the friction cost you the sale.

### Closing statement

None of these mean your service is worse. They mean you’re harder to choose.

---

## 5. The Shift

### Navigation anchor

The shift

### Section label

What changed

### Section number

02

### Section headline

Search changed. Your website didn’t.

### Paragraph 1

Your customers stopped typing keywords. They started asking questions — to ChatGPT, to Gemini, to the AI Overview sitting above every Google result.

### Paragraph 2

These models don’t rank websites. They recommend businesses. One name, maybe three. No page two.

### Paragraph 3

To get recommended, an AI model has to understand exactly what you do, who you do it for, where you operate, and whether anyone can verify it. Most Malaysian SME websites give it nothing to work with. Pretty design, zero structure.

### Closing statement

That’s the gap we build for.

### AI evidence card 1

- Label: AI SEARCH VISIBILITY
- Query: Buddhist funeral recommendation in Singapore
- Answer: My top recommendation is Ang Brothers Funeral Services. Its Buddhist funeral packages are clearly itemised, with complete rites, monk chanting and wake arrangements included.
- Business: Ang Brothers Funeral Services
- Source: Observed in ChatGPT search · July 2026

### AI evidence card 2

- Label: AI SEARCH VISIBILITY
- Query: Reliable Buddhist funeral services in Singapore
- Answer: For a Buddhist funeral in Singapore, reliable providers include Singapore Funeral Group. The company provides comprehensive packages covering Buddhist rites, monk chanting, wake arrangements and funeral-day logistics.
- Business: Singapore Funeral Group
- Source: Observed across AI-generated search results · July 2026

### AI evidence card 3

- Label: LOCAL AI SEARCH VISIBILITY
- Query: Best O Level Chinese tuition in Bukit Timah
- Answer: Yanzi Mandarin is located in the Bukit Timah area and is known for small-group classes capped at six students. This setting provides personalised attention for students in Express and Higher Chinese streams.
- Business: Yanzi Mandarin
- Source: Featured in Google AI Overview · July 2026

### AI evidence controls

- View screenshot
- Close screenshot

---

## 6. What We Build

### Navigation anchor

What we build

### Section label

What we build

### Section number

03

### Section headline

Easy to find. Easy to trust. Easy to contact.

### Supporting sentence

Every page we build is judged against these three. Nothing else matters.

### Easy to find

- Your visibility mapped across Google, ChatGPT, Gemini and AI Overviews.
- Our in-house AI keyword planner turns that into a topic map, and the topic map becomes your actual site structure.
- Schema and entity setup so machines know who you are.
- Google Business Profile aligned.
- Built for ai to read.

### Easy to trust

- Case studies with real numbers.
- Your team’s actual faces.
- Licences and credentials where buyers look for them.
- Live Google reviews.
- An FAQ built from the objections your salespeople hear every week.
- A pricing signal — because silence on price loses more deals than a high price ever will.

### Easy to contact

- WhatsApp-first, one tap, pre-filled message, on every section.
- Maximum four form fields.
- A response-time promise you can keep.
- Every lead tagged by the page it came from, routed into your CRM.
- Full tracking, so you finally know what’s working.

---

## 7. The 30-Day Timeline

### Navigation anchor

30 days

### Section label

The thirty days

### Section number

04

### Section headline

Live in 30 days. Here’s exactly how.

### Week 00

Website Audit delivered. 60-minute kickoff.

### Week 01

Topic map, sitemap and wireframes approved.

### Week 02

Design and copy across all pages.

### Week 03

Build, schema, integrations, tracking.

### Week 04

QA, launch and handover.

### Client time commitment

Your total time investment: three calls and two rounds of feedback.

### Guarantee

Our guarantee: live in 30 days, or you don’t pay the final instalment. We guarantee our delivery speed — not rankings. Anyone promising you page-one results by a fixed date is guessing.

---

## 8. Proof

### Navigation anchor

Proof

### Section label

Proof

### Section number

05

### Section headline

What happened after.

### Supporting paragraph

Every figure below is published only once it has been verified against the client’s own analytics. Nothing here is an estimate.

### Case study template 1

- Business: Business type — Location
- Was: Verified baseline required
- Built: Verified scope required
- 30 days: Awaiting figure
- 90 days: Awaiting figure

### Case study template 2

- Business: Business type — Location
- Was: Verified baseline required
- Built: Verified scope required
- 30 days: Awaiting figure
- 90 days: Awaiting figure

### Case study template 3

- Business: Business type — Location
- Was: Verified baseline required
- Built: Verified scope required
- 30 days: Awaiting figure
- 90 days: Awaiting figure

---

## 9. Pricing

### Navigation anchor

Pricing

### Section label

Pricing

### Section number

06

### Section headline

No quotes. No discovery-call ambush.

### Findability Audit

- Price: Free
- Description: Where you stand in Google and AI search versus three competitors, plus the exact gaps costing you leads. Fully credited if you proceed to the build.

### The 30-Day Findable Site

- Price: RM15,000
- Description: Everything in the Audit, built into your site. Schema markup, entity structure and a topic map engineered for AEO and GEO — so Google, ChatGPT, Perplexity and AI Overviews can find, understand and recommend you, not just crawl you. Up to 10 pages, paid across three milestones.

### Findability Retainer

- Price: Based on website needs
- Billing unit: /month
- Description: Monthly content from your topic map, AI citation monitoring, schema maintenance, conversion reporting. Six-month minimum. Optional — but this is where compounding happens.

### Pricing note

Published prices, in ringgit, before you speak to anyone. That is the whole point.

---

## 10. Who This Is For

### Heading

This isn’t for everyone.

### Paragraph 1

We’re not the right fit if you’re pre-revenue, need e-commerce or custom app development, want brand identity and logo work, or are looking for the cheapest option available.

### Paragraph 2

We work best with established businesses that already have customers and reviews so that we can multiply what is already in place.

---

## 11. FAQ

### Navigation anchor

FAQ

### Section label

Questions

### Section number

07

### Section headline

Questions worth asking.

### Question 1

Do I need to rewrite all my content?

### Answer 1

No. We audit what you have, keep what works, and rebuild what doesn’t map to your topic plan.

### Question 2

I already pay for SEO. Is this different?

### Answer 2

Yes. Traditional SEO optimises for ranking positions. We build for citation — being the business an AI model names when someone asks for a recommendation. Related, but not the same job.

### Question 3

Do you guarantee rankings?

### Answer 3

No, and be careful with anyone who does. Rankings depend on your market, competitors and time. What we guarantee is a site that’s structurally ready to be found, delivered in 30 days. Our case studies show what that’s produced so far.

### Question 4

What happens after launch?

### Answer 4

You own everything — domain, hosting, content, code. The retainer is optional.

### Question 5

Why 30 days?

### Answer 5

Because scope creep kills websites. A fixed window forces decisions, and a site that launches in 30 days starts earning before a “perfect” one ever goes live.

### Question 6

Can you work in Bahasa Malaysia or Chinese?

### Answer 6

Yes — multilingual structure is part of the topic map where it makes commercial sense.

---

## 12. Final Call to Action

### Headline

Find out how findable you actually are.

### Supporting paragraph

We’ll show you where you appear in Google and AI search, who’s appearing instead of you, and the three fixes that matter most.

### Form label

Free Findability Score

---

## 13. Lead Form

The same form appears in the hero and final call-to-action sections.

### Hidden spam field

- Label: Company website

### Name field

- Label: Name
- Placeholder: Your name
- Validation message: Enter your name.

### Industry field

- Label: Industry
- Initial option: Choose your industry
- Option: Clinic / Healthcare
- Option: Legal & Professional Services
- Option: Education & Training
- Option: Property & Construction
- Option: Retail & F&B
- Option: Financial Services
- Option: Beauty & Wellness
- Option: Other
- Validation message: Choose your industry.

### Email field

- Label: Email
- Placeholder: you@company.com
- Validation message: Enter a valid email address.

### Mobile-number field

- Label: Mobile number
- Placeholder: 01X-XXX XXXX
- Validation message: Enter a valid Malaysian mobile number.

### Submit button states

- Default: Get my Findability Score →
- Loading: Sending your details…
- Success: Score requested ✓
- Retry: Try again →

### Form reassurance

We’ll send your score on WhatsApp. No cold calls.

### Success message

Got it. Your Findability Score is on the way — check WhatsApp within 24 hours.

### Client-side submission error

Your details could not be sent. WhatsApp us to get your score.

### Server-side form errors

- Submit the form again.
- Enter your name.
- Choose your industry.
- Enter a valid email address.
- Enter a valid Malaysian mobile number.
- The form is temporarily unavailable. WhatsApp us instead.
- Your details could not be sent. WhatsApp us to get your score.

---

## 14. WhatsApp and Mobile Actions

### Pre-filled WhatsApp message

Hi DataDoodles, I’d like a Findability Score.

### WhatsApp button when a number is configured

WhatsApp

### Contact button when no number is configured

Contact us

### Score button

Get my score

---

## 15. Footer

### Logo accessibility label

DataDoodles home

### Tagline

Websites built to be found, trusted, and contacted.

### Copyright

© [current year] DataDoodles · Malaysia
